...
#define SliderInt RaaxGUI::SlderInt
...
#define SliderInt ImGui::SliderInt
...
static void VisualsTab() {
    ...
    if (BeginChild("ConfigRegion", ImVec2(0, 0), ImGuiChildFlags_Borders)) {
        ...
        if (BeginTabItem("Player")) {
            ...
            Checkbox("Chinese Hat", &Config.Player.ChineseHat);
            if (Config.Player.ChineseHat) {
                SliderFloat("Hat Radius##ch", &Config.Player.ChineseHatRadius, 5.f, 100.f);
                SliderFloat("Hat Height##ch", &Config.Player.ChineseHatHeight, 2.f, 60.f);
                SliderInt("Hat Segments##ch", &Config.Player.ChineseHatSegments, 8, 96);
            }

            Checkbox("Head Circle", &Config.Player.HeadCircle);
            if (Config.Player.HeadCircle) {
                SliderFloat("Head Circle Radius", &Config.Player.HeadCircleRadius, 1.f, 25.f);
            }
        ...
        if (BeginTabItem("Radar")) {
            ...
            SliderFloat("Player Dot Size", &Config.Radar.PlayerDotSize, 0.5f, 10.f);
...
static void MiscTab() {
    ...
    if (BeginChild("ConfigRegion", ImVec2(0, 0), ImGuiChildFlags_Borders)) {
        ...
        Text("Made by U_Know_69");
        Text("This GUI of 1hAck was made by Maslo2116 so thank you<3");
        Text("This was Raax v2 and me(U_Know_69) made it work for Retrac ");
...
void Tick() {
    ...
    if (GUI::MainWindow::g_WindowOpen) {
        ...
        float guiScale = Config::g_Config.GuiScale;
        if (guiScale < 0.5f) guiScale = 0.5f;
        else if (guiScale > 2.0f) guiScale = 2.0f;
        io.FontGlobalScale = guiScale;

        // Accent color
        const ImVec4 accent = ImVec4(0.88f, 0.22f, 0.22f, 1.00f);
        const ImVec4 accent_hover = ImVec4(0.98f, 0.26f, 0.26f, 1.00f);
        ...
        style.Colors[ImGuiCol_TabActive] = ImVec4(0.13f, 0.13f, 0.13f, 1.00f);
        ...
        // Main window
        ImGui::SetNextWindowSize(ImVec2(648 * io.FontGlobalScale, 378 * io.FontGlobalScale), ImGuiCond_Once);
        ImGui::SetNextWindowSizeConstraints(ImVec2(420, 260), ImVec2(io.DisplaySize.x, io.DisplaySize.y));
        if (ImGui::Begin("Cool.Cheats.Server<3", nullptr,
            ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoSavedSettings)) {

            // Auto-scale text by window size so resizing the window adjusts text size
            static ImVec2 s_baseWindowSize = ImVec2(648, 378);
            static bool   s_baseSizeInitialized = false;
            ImVec2 currentSize = ImGui::GetWindowSize();
            if (!s_baseSizeInitialized && currentSize.x > 0.0f && currentSize.y > 0.0f) {
                s_baseWindowSize = currentSize;
                s_baseSizeInitialized = true;
            }
            float widthRatio = currentSize.x > 0.0f ? (currentSize.x / s_baseWindowSize.x) : 1.0f;
            float heightRatio = currentSize.y > 0.0f ? (currentSize.y / s_baseWindowSize.y) : 1.0f;
            float autoScale = (widthRatio + heightRatio) * 0.5f; // geometric approx without sqrt
            float newScale = io.FontGlobalScale * autoScale;
            if (newScale < 0.5f) newScale = 0.5f;
            else if (newScale > 3.0f) newScale = 3.0f;
            io.FontGlobalScale = newScale;

            ImGui::Columns(2, "MainColumns", false);
            ImGui::SetColumnWidth(0, 126.0f * io.FontGlobalScale);

            BeginChild("Tabs", ImVec2(0, 0), true, ImGuiWindowFlags_NoScrollbar);
            {
                const char* tabNames[] = { "Aimbot", "Trigger Bot", "Visuals", "Exploits", "Keybinds", "Config", "Misc" };
                ...
                for (int i = 0; i < IM_ARRAYSIZE(tabNames); i++) {
                    ...
                    // Style buttons so selected items are drawn with our own overlay (transparent base)
                    ImGui::PushStyleColor(ImGuiCol_Button, isSelected ? ImVec4(0, 0, 0, 0) : style.Colors[ImGuiCol_Button]);
                    ImGui::PushStyleColor(ImGuiCol_ButtonHovered, isSelected ? ImVec4(0, 0, 0, 0) : style.Colors[ImGuiCol_ButtonHovered]);
                    ImGui::PushStyleColor(ImGuiCol_ButtonActive, isSelected ? ImVec4(0, 0, 0, 0) : style.Colors[ImGuiCol_ButtonActive]);
                    int stylePushed = 3;
                    if (isSelected) {
                        // Make default button text invisible; we'll draw it ourselves
                        ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0, 0, 0, 0));
                        stylePushed++;
                    }
                    if (Button(tabNames[i], ImVec2(-1, 32.0f * io.FontGlobalScale))) {
                        selectedTab = i;
                    }
                    ...
                    if (isSelected) {
                        // Gradient fill from accent to dark
                        ImColor left_col = ImColor(accent.x, accent.y, accent.z, accent.w);
                        ImColor right_col = ImColor(0.05f, 0.05f, 0.05f, 1.0f);
                        draw_list->AddRectFilledMultiColor(p_min, p_max, left_col, right_col, right_col, left_col);

                        // Left accent bar
                        draw_list->AddRectFilled(ImVec2(p_min.x, p_min.y), ImVec2(p_min.x + 3.0f, p_max.y), left_col);

                        // Label (centered) with high contrast
                        ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.97f, 0.97f, 0.97f, 1.00f));
                        ImGui::RenderTextClipped(p_min, p_max, tabNames[i], NULL, NULL, ImVec2(0.5f, 0.5f));
                        ImGui::PopStyleColor();
                    }
            ...
            BeginChild("Content", ImVec2(0, 0), false);
            {
                ...
                case 1: TriggerBotTab(); break;
                case 2: VisualsTab(); break;
                case 3: ExploitsTab(); break;
                case 4: KeybindsTab(); break;
                case 5: ConfigTab(); break;
        ...
        // Cursor 
#ifndef _ENGINE
        ImGui::GetForegroundDrawList()->AddCircle(io.MousePos, 4.f, ImColor(0.f, 0.f, 0.f));
        ImGui::GetForegroundDrawList()->AddCircle(io.MousePos, 3.f, ImColor(1.f, 1.f, 1.f));
#else
        SDK::FVector2D MousePos = RaaxGUI::Impl::GetMousePos();
        Drawing::Circle(MousePos, 4.f, 12, SDK::FLinearColor::White, 3.f);
#endif
    }
}
